...
def appname = 'jump-boot-service'
def ocp_project='example'
def version = '0.0.1'
def image = 'quay.io/centos7/nodejs-14-centos7'
def reponame= 'https://github.com/jefripunza/jump-boot.git'
pipeline {
    agent { node { label 'nodejs' } }
    stages {
        stage("Code Checkout from GitLab") {
            steps {
                git url: "${reponame}", branch: "${branchrepo}", credentialsId: 'jefripunza'
            }
        }
        stage ('OpenShift Login') {
            steps {
                ...
                usernameVariable: 'oc_username', passwordVariable: 'oc_password']]) {
                    sh 'oc login -u=${oc_username} -p=${oc_password} https://example.com:8443/ --insecure-skip-tls-verify=true'
                }
        ...
        stage ('openshift create deployment'){
            steps {
                sh "oc new-app ${image}~${reponame}#${branchrepo} --name ${appname} -n marketplace --labels='project=example' || echo 'app exists'"
				sh "sleep 30"
